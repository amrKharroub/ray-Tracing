...
Color trace(Vector pos, Vector dir, int step)
{
	...
	if ((s.index > -1 && s.dist < lightDist) || lDotn <= 0) {
		...
		//attenuation is multiplyed to have a brighter image
		float att = 1000.0f / (1 + 2 * lightDist + 1 * lightDist * lightDist);
		att *= lDotn; //lighting based on angle
	...
	if (shininess != 0.0f)
	{
		...
		Color rColor = trace(q.point, rdir, ++step);
	...
	if (translucency != 0.0f)
	{
		...
		Color rColor = trace(q.point, refracted, ++step);
...
char* display() {
	...
	float x1, y1, xc, yc;
	Vector eye(0., 0., 5.0f);
	int size = (int)(WIDTH * PPU * HEIGHT * PPU * 3);
	char* imageData = new char[size];
	int imagepos = 0;
	for (int i = 0; i < heightInPixels; i++)	//Scan every "pixel"
	{
		std::cerr << "\rScanlines remaining: " << heightInPixels - i << ' ' << std::flush;
		y1 = YMAX - i * pixelSize; //start from top-left corner
		yc = y1 + halfPixelSize;
		for (int j = 0; j < widthInPixels; j++)
		{
			...
			imageData[imagepos++] = max(0.0f, min(col.r, 1.0f)) * 255;
			imageData[imagepos++] = max(0.0f, min(col.g, 1.0f)) * 255;
			imageData[imagepos++] = max(0.0f, min(col.b, 1.0f)) * 255;
		}
	}
	std::cerr << "\nDone\n";
	return imageData;
...
void initialize()
{
	...
	Sphere* sphere1 = new Sphere(Vector(10, 0.0f, -40), 4.5, Color::CYAN, 0.5f, 0.5f);
...
{
	char filename[16] = "TracedImage.png";
	int width = WIDTH * PPU;
	int height = HEIGHT * PPU;
	initialize();
	char* image;
	image = display();
